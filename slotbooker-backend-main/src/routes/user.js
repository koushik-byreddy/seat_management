...
const { updateValidator, registerValidator, loginValidator } = userValidators;
    ...
    try {
      ...
      if (!otpRecord) {
        return res.status(400).json({
          ...
          error: "No OTP found for this email. Please request OTP again.",
        });
      }
      ...
      if (otpRecord.otp !== otp) {
        return res
          .status(400)
          .json({ code: "user_registration_failed", error: "Invalid OTP" });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (otpRecord.otp !== otp) {
        return res
          .status(400)
          .json({ code: "reset_password_failed", error: "Invalid OTP" });
      }
...
userRouter.post("/verify/email", async (req, res) => {
  ...
  try {
    ...
    if (userExists) {
      res
        .status(200)
        .json({ code: "user_verification_success", isUserRegistered: true });
    } else {
      res
        .status(404)
        .json({ code: "user_verification_failed", isUserRegistered: false });
    }
  } catch (error) {
    res.status(500).json({ code: "user_registration_failed", error: error });
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user)
        return res
          .status(400)
          .json({ code: "user_login_failed", error: "Invalid credentials" });
...
userRouter.post("/verify/otp", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (otpRecord.otp !== otp) {
      return res
        .status(400)
        .json({ code: "otp_verification_failed", error: "Invalid OTP." });
    }
    ...
    res
      .status(200)
      .json({ code: "otp_verification_success", isVerified: true });
  } catch (error) {
...
userRouter.post("/verify", authenticateJWT, async (req, res) => {
  ...
  if (!user)
    return res
      .status(400)
      .json({ code: "user_verification_failed", error: "Invalid user" });
  try {
    res.status(200).json({
...
userRouter.get("/all", async (req, res) => {
  ...
  res.status(200).json({ all, allOtp });
});
